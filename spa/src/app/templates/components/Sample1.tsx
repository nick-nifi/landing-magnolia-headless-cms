...
interface ISample1Props {
  title: string;
  description: string;
  imageChooser?: ImageChooser;
}

const Sample1: React.FC<ISample1Props> = ({ title, description, imageChooser }) => {
  let imageSrc = '';
  let imageAlt = 'Image';
    ...
    <div className="w-full p-4">
      <div className="rounded overflow-hidden shadow-lg bg-white">
        ...
        <div className="p-6">
          ...
          {description && <div dangerouslySetInnerHTML={{ __html: decodeIfEscaped(description) }}/>}
        </div>
      </div>
    </div>
  );
